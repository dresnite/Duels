...
class DuelsRefuseCommand implements DuelsCommand {
    
    /** @var Duels */
    private $loader;
    
    /**
     * DuelsRefuseCommand constructor.
    ...
    public function onCommand(Session $session, array $args = []): void {
        if($session->hasMatch()) {
            ...
            if($lastOwner != null) {
                ...
                $session->setLastInvitationOwner(null);
                $session->sendLocalizedMessage("INVITATION_REFUSED", [
                    "name" => $lastOwner
                ]);
                $lastOwner->sendLocalizedMessage("YOUR_INVITATION_WAS_REFUSED", [
                    "name" => $session
                ]);
            } else {
        ...
        if($playerSession->hasMatch()) {
            ...
            $playerSession->sendLocalizedMessage("YOUR_INVITATION_WAS_REFUSED", [
                "name" => $session
            ]);
        } else {
